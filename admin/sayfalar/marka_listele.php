...
{
	$id = g('id');
	$resim_bul=Sonuc(Sorgu("SELECT * FROM markalar WHERE id='$id'"));
	$resim_sil=unlink("../uploads/markalar/".$resim_bul->resim);
	$resim_sil2=unlink("../uploads/markalar/thumb/".$resim_bul->resim);
	
	$sayfa_sil_sorgu = Sorgu("DELETE FROM markalar WHERE id='$id'");
...
<div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            <small><i class="fa fa-edit"></i> Çalışan Listesi</small>
          </h1>
          <ol class="breadcrumb">
          ...
          <div class="row">
            <div class="col-xs-12">
              <div class="box">
                ...
                <a href="marka-ekle.html" class="btn bg-navy margin"><i class="fa fa-plus"></i> Çalışan Ekle</a>
                </div><!-- /.box-header -->
                <div class="box-body">
                  <table id="example1" class="table table-bordered table-hover">
                    ...
                    <tbody>
        <?php $SayfaSorgu = Sorgu("SELECT * FROM markalar ORDER BY id DESC");
        while($SayfaSonuc = Sonuc($SayfaSorgu)){?>
                      <tr>
                        <td><?php echo $SayfaSonuc->id; ?></td>
						...
						<?php if($SayfaSonuc->resim == ""){?>
			 			...
			 			<?php }else{?>
			 			<a class="btn btn-success btn-xs" href="../uploads/markalar/<?php echo $SayfaSonuc->resim; ?>" target="_blank"><i class="fa fa-eye"></i> Resmi Gör</a>
						 <?php } ?>
                         </td>
                        <td>
						<?php
			 ...
			 <a href="?sayfa=marka-ekle&islem=duzenle&id=<?php echo $SayfaSonuc->id;?>" class="btn btn-info btn-xs" title="Düzenle" id="add-sticky">
			 Düzenle
			 </a>
                        </td>
                      </tr>
